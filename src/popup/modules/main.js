// main.js - 主入口

import { state, initElements } from './state.js';
import * as theme from './theme.js';
import * as settings from './settings.js';
import * as sessionManager from './sessionManager.js';
import { setupEventListeners } from './events.js';
import { initI18n } from './i18n.js';
import { updateAllTexts, updateTrackingUI } from './i18nUI.js';
import { initSearch } from './search.js';
import { initTreeDragDrop } from './dragDrop.js';
import { initKeyboard } from './keyboard.js';

const VISUAL_SETTING_KEYS = [
  'showFavicons',
  'defaultExpand',
  'sortMode',
  'viewMode',
  'language',
  'colorTheme'
];

function hasVisualSettingsChange(change) {
  if (!change) return false;
  return VISUAL_SETTING_KEYS.some(key => change.oldValue?.[key] !== change.newValue?.[key]);
}

/**
 * 诊断工具：输出当前状态
 */
function diagnose() {
  console.log('[MindGit 诊断] 当前状态:', {
    currentSessionId: state.currentSessionId,
    sessionCount: Object.keys(state.currentSessions).length,
    sessions: state.currentSessions,
    settings: state.currentSettings
  });
}

/**
 * 初始化应用
 */
async function init() {
  // 初始化 DOM 元素引用
  initElements();

  // 加载设置和语言，避免初始化阶段重复写入 settings。
  await settings.loadSettings();
  await initI18n();

  // 更新界面文本
  updateAllTexts();

  // 加载主题
  await theme.loadTheme();

  // 监听存储变化（在加载数据前注册）
  const { onStorageChanged } = await import('./api.js');
  const gistSync = await import('./gistSync.js');
  onStorageChanged((changes) => {
    if (changes.settings?.newValue) {
      state.currentSettings = {
        ...state.currentSettings,
        ...changes.settings.newValue
      };
      updateTrackingUI();
    }
    if (changes.sessions || changes.currentSession || hasVisualSettingsChange(changes.settings)) {
      sessionManager.checkAndRefresh();
    }
    if (changes.sessions || changes.currentSession) {
      gistSync.scheduleAutoSync();
    }
  });

  // 加载会话列表
  await sessionManager.loadSessions();

  // 设置事件监听
  setupEventListeners();

  // 初始化搜索功能
  initSearch();

  // 初始化树形拖拽功能（容器事件）
  initTreeDragDrop();

  // 初始化快捷键系统
  initKeyboard();

  // 初始化云端同步（延迟，等设置加载完成）
  gistSync.initAutoSync();

  // 应用会话列表初始折叠状态
  if (state.isSessionListExpanded) {
    state.elements.sessionListContainer.classList.add('expanded');
  } else {
    state.elements.sessionListContainer.classList.remove('expanded');
  }
  
  // 尝试自动创建会话
  await sessionManager.tryAutoCreateSession();

  // 将诊断函数暴露到全局，方便在控制台调用
  window.mindGitDiagnose = diagnose;
}

// DOM 加载完成后初始化
document.addEventListener('DOMContentLoaded', init);
